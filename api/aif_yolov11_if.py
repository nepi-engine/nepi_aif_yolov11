...
class Yolov11AIF(object):
    ...
    def __init__(self, aif_dict,launch_namespace, mgr_namespace, models_lib_path):
      ...
      nepi_sdk.log_msg_info("Yolov11 models path: " + self.models_folder_path)
    ...
    def getModelsDict(self):
        ...
        if os.path.exists(self.models_folder_path) == False:
            ...
            nepi_sdk.log_msg_info(self.log_name + ": Found model info files: " + str(self.cfg_files))
            ...
            for f in self.cfg_files:
                ...
                #nepi_sdk.log_msg_warn(self.log_name + ": Checking that model type is supported: " + weight_file_path + " for model name " + model_type)
                type_keys =self.node_file_dict.keys()
                #nepi_sdk.log_msg_warn(self.log_name + ": Checking that model type is supported: " + weight_file_path + " for model name " + str(type_keys)
    ...
    def loadModel(self, model_dict):
        ...
        nepi_sdk.log_msg_info(self.log_name + ": Launching Yolov11 AI node " + model_name + " with commands: " + str(launch_cmd_line))
    ...
    def killModel(self,model_name):
        if model_name in self.ai_node_dict.keys():
            ...
            nepi_sdk.log_msg_info(self.log_name + ": Killing Yolov11 AI node: " + model_name)
