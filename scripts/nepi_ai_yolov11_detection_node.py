        ...
        if self.weight_file_path == "":
            ...
            if model_info == "":
                ...
                if model_framework != 'yolov11':
                    self.msg_if.pub_warn("Model not a yolov11 model: " + model_framework)
            ...
            if 'cv2_img' in img_dict.keys():
                ...
                if cv2_img is not None:
                    ...
                    try:
                        ...
                        #self.msg_if.pub_warn("Got Yolov11 detection ids: " + str(ids))
                        boxes = results[0].boxes.xyxy.to('cpu').tolist()
                        #self.msg_if.pub_warn("Got Yolov11 detection boxes: " + str(boxes))
                        confs = results[0].boxes.conf.to('cpu').tolist()
                        #self.msg_if.pub_warn("Got Yolov11 detection confs: " + str(confs))
