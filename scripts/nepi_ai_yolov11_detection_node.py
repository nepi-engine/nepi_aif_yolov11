    ...
    def __init__(self):
        ...
        if self.weight_file_path == "":
            ...
            if model_info == "":
                ...
                if model_framework != 'yolov11':
                    nepi_msg.publishMsgWarn(self,"Model not a yolov11 model: " + model_framework)
            ...
            if 'cv2_img' in img_dict.keys():
                ...
                if cv2_img is not None:
                    ...
                    try:
                        ...
                        #nepi_msg.publishMsgWarn(self,"Got Yolov11 detection results: " + str(results[0].boxes))
                
                        #nepi_msg.publishMsgWarn(self,"Got Yolov11 detection results: " + str(results[0].boxes))
                        ids = results[0].boxes.cls.to('cpu').tolist()
                        #nepi_msg.publishMsgWarn(self,"Got Yolov11 detection ids: " + str(ids))
                        boxes = results[0].boxes.xyxy.to('cpu').tolist()
                        #nepi_msg.publishMsgWarn(self,"Got Yolov11 detection boxes: " + str(boxes))
                        confs = results[0].boxes.conf.to('cpu').tolist()
                        #nepi_msg.publishMsgWarn(self,"Got Yolov11 detection confs: " + str(confs))
