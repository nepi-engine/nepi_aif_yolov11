...
class Yolov11AIF(object):
    ...
    def __init__(self, ai_dict,launch_namespace, mgr_namespace, models_lib_path):
      ...
      nepi_msg.printMsgInfo("Yolov11 models path: " + self.models_folder_path)

    
    #################
    # Yolov11 Model Functions

    def getModelsDict(self):
        models_dict = dict()
        # Try to obtain the path to Yolov11 models from the system_mgr
        nepi_msg.printMsgInfo("ai_yolov11_if: Looking for model files in folder: " + self.models_folder_path)
        # Grab the list of all existing yolov11 cfg files
        if os.path.exists(self.models_folder_path) == False:
            nepi_msg.printMsgInfo("ai_yolov11_if: Failed to find models folder: " + self.models_folder_path)
            return models_dict
        else:
            ...
            for f in self.cfg_files:
                ...
                success = False
                try:
                    #nepi_msg.printMsgWarn("ai_yolov11_if: Opening yaml file: " + f) 
                    yaml_stream = open(f, 'r')
                    success = True
                    #nepi_msg.printMsgWarn("ai_yolov11_if: Opened yaml file: " + f) 
                except Exception as e:
                    nepi_msg.printMsgWarn("ai_yolov11_if: Failed to open yaml file: " + str(e))
                if success:
                    try:
                        # Validate that it is a proper config file and gather weights file size info for load-time estimates
                        #nepi_msg.printMsgWarn("ai_yolov11_if: Loading yaml data from file: " + f) 
                        cfg_dict = yaml.load(yaml_stream)  
                        model_keys = list(cfg_dict.keys())
                        model_key = model_keys[0]
                        #nepi_msg.printMsgWarn("ai_yolov11_if: Loaded yaml data from file: " + f) 
                    except Exception as e:
                        nepi_msg.printMsgWarn("ai_yolov11_if: Failed load yaml data: " + str(e)) 
                        success = False 
                try: 
                    #nepi_msg.printMsgWarn("ai_yolov11_if: Closing yaml data stream for file: " + f) 
                    yaml_stream.close()
                except Exception as e:
                    nepi_msg.printMsgWarn("ai_yolov11_if: Failed close yaml file: " + str(e))
                
                if success == False:
                    nepi_msg.printMsgWarn("ai_yolov11_if: File does not appear to be a valid A/I model config file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn("ai_yolov11_if: Import success: " + str(success) + " with cfg_dict " + str(cfg_dict))
                cfg_dict_keys = cfg_dict[model_key].keys()
                #nepi_msg.printMsgWarn("ai_yolov11_if: Imported model key names: " + str(cfg_dict_keys))
                #nepi_msg.printMsgWarn("ai_yolov3_if: Imported model key names: " + str(cfg_dict_keys))
                if ("framework" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn("ai_yolov11_if: Framework does not specified in model yaml file: " + f + "... not adding this model")
                    continue
                ...
                if framework != 'yolov11':
                    nepi_msg.printMsgWarn("ai_yolov11_if: Model " + model_name + " not a yolov3 model" + framework + "... not adding this model")
                    continue

                weight_file = cfg_dict[model_key]["weight_file"]["name"]
                weight_file_path = os.path.join(self.models_folder_path,weight_file)
                #nepi_msg.printMsgWarn("ai_yolov11_if: Checking that model weights file exists: " + weight_file_path + " for model name " + model_name)
                if not os.path.exists(weight_file_path):
                    nepi_msg.printMsgWarn("ai_yolov11_if: Model " + model_name + " specifies non-existent weights file " + weight_file_path + "... not adding this model")
                    continue
                model_type = cfg_dict[model_key]['type']['name']
                #nepi_msg.printMsgWarn("ai_yolov11_if: Checking that model type is supported: " + weight_file_path + " for model name " + model_type)
                type_keys =self.node_file_dict.keys()
                #nepi_msg.printMsgWarn("ai_yolov11_if: Checking that model type is supported: " + weight_file_path + " for model name " + str(type_keys)
                if model_type not in self.node_file_dict.keys():
                    nepi_msg.printMsgWarn("ai_yolov11_if: Model " + model_name + " specifies non-supported model type " + model_type + "... not adding this model")
                    continue
                else:
    ...
    def loadModel(self, model_dict):
        ...
        nepi_msg.printMsgInfo("ai_yolov11_if: Launching Yolov11 AI node " + model_name + " with commands: " + str(launch_cmd_line))
    ...
    def killModel(self,model_name):
        if model_name in self.ai_node_dict.keys():
            ...
            nepi_msg.printMsgInfo("ai_yolov11_if: Killing Yolov11 AI node: " + model_name)
...
if __name__ == '__main__':
    ...
    while nepi_ros.check_for_node(node_name):
        nepi_msg.printMsgInfo("ai_yolov11_if: Trying to kill running node: " + node_name)
